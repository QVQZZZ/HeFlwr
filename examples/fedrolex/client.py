...
class FlClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        net_struct = eval(config['net_struct'])
        self.net = self.net_class(net_struct).to(DEVICE)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p', type=str, help='FedRolex p, such as 1/4.')
    ...
    if dataset == "cifar10":
        net = ResNet18(net_struct=('0', p)).to(DEVICE)
        net_class = ResNet18
    elif dataset == "mnist":
        net = LeNet(net_struct=('0', p)).to(DEVICE)
        net_class = LeNet
    ...
    monitor = FileMonitor(file='./fedrolex_test_log.txt')
    monitor.start()
