...
class FlClient(fl.client.NumPyClient):
    def __init__(self, cid, net, train_loader, test_loader, num_examples, net_class):
        self.cid = cid
        self.net = net
    ...
    def fit(self, parameters, config):
        ...
        self.net = self.net_class(net_type=net_type).to(DEVICE)
...
if __name__ == '__main__':
    ...
    if dataset == "cifar10":
        net = ResNet18(0).to(DEVICE)
        net_class = ResNet18
    elif dataset == "mnist":
        net = LeNet(0).to(DEVICE)
        net_class = LeNet
    ...
    client = FlClient(cid, net, train_loader, test_loader, num_examples, net_class)

    monitor = FileMonitor(file='./mfl_test_log.txt')
    monitor.start()
